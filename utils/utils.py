...
def wrap_backbone_with_checkpointing(model, segments=4):
    """
    Replaces encoder.backbone.blocks with a checkpointed sequential.
    """
    blocks = model.encoder.backbone.blocks
    # turn nn.Sequential([...]) into list[Module] if needed
    if isinstance(blocks, torch.nn.Sequential):
        blocks = list(blocks)
    # cut roughly equal segments for uniform memory
    seg_len = max(1, len(blocks) // segments)
    model.encoder.backbone.blocks = checkpoint_sequential(blocks, segments, seg_len)
